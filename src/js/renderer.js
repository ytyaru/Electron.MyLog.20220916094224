window.addEventListener('DOMContentLoaded', async(event) => {
    ...
    function setLineOfChars() { // 一行あたりの字数（20,25,30,35,38）
        ...
        /*
        const max = Math.min(40, parseInt(w/(16*screenCount)))
        let rec = Math.min(38, parseInt(w/(21*screenCount)))
        //rec = (rec < 20) ? max : rec
        rec = (20 < max) ? rec : max
        let min = Math.max(max-10, parseInt(w/(32*screenCount)))
        //min = (max < 20) ? max : min
        min = (20 < max) ? min : max
        */
        ...
        const min = (20 < lineOfChars) ? lineOfChars - 5 : 20
        const max = (lineOfChars < 38) ? lineOfChars + 5 : 38
        document.getElementById('line-of-chars').min = min
        document.getElementById('line-of-chars').max = max
        document.getElementById('line-of-chars').value = lineOfChars
        document.getElementById('line-of-chars-min').innerText = min
        //document.getElementById('line-of-chars-rec').innerText = lineOfChars
        document.getElementById('line-of-chars-now').innerText = lineOfChars
        document.getElementById('line-of-chars-max').innerText = max
        console.log('min,now,max',min, lineOfChars, max)
        //console.log('now,w',lineOfChars, w)
        console.log(`%c${lineOfChars}字／行　${w}px`, `color:red; font-size:16px;`)
        */
